...
highp float fSubtraction(precision highp float a, precision highp float b) {return max(-a,b);}
precision highp float fIntersection(precision highp float d1, precision highp float d2) {return max(d1,d2);}
void fUnion(inout precision highp float d1, precision highp float d2) {d1 = min(d1,d2);}
precision highp float pSphere(vec3 p, precision highp float s) {return length(p)-s;}
precision highp float pRoundBox(vec3 p, vec3 b, precision highp float r) {return length(max(abs(p)-b,0.0))-r;}
precision highp float pTorus(vec3 p, vec2 t) {vec2 q = vec2(length(p.xz)-t.x,p.y); return length(q)-t.y;}
precision highp float pTorus2(vec3 p, vec2 t) {vec2 q = vec2(length(p.xy)-t.x,p.z); return length(q)-t.y;}
precision highp float pCapsule(vec3 p, vec3 a, vec3 b, precision highp float r) {vec3 pa = p - a, ba = b - a;
	precision highp float h = clamp( dot(pa,ba)/dot(ba,ba), 0.0, 1.0 ); return length( pa - ba*h ) - r;}

precision highp float distf(vec3 p)
{
	precision highp float d = 100000.0;
...
vec3 normal(vec3 p)
{
	const precision highp float eps = 0.01;
	precision highp float float m;
...
{
	const precision highp float maxDist = 200.0;
	const precision highp float minDist = 0.001;
	const int maxIter = RAYMARCH_ITERATIONS;
	
	precision highp float dist = 0.0;
	
	precision highp float lastDistEval = 1e10;
	precision highp float edge = 0.0;
	
	for(int i = 0; i < maxIter; i++) {
		vec3 pos = (from + increment * dist);
		precision highp float distEval = distf(pos);
	...
	precision highp float mat = 1.0;
	if (dist >= maxDist) mat = 0.0;
	
	return vec4(dist, mat, edge, 0);
}

precision highp float shadow(vec3 from, vec3 increment)
{
	const precision highp float minDist = 1.0;
	
	precision highp float res = 1.0;
	precision highp float t = 1.0;
	for(int i = 0; i < SHADOW_ITERATIONS; i++) {
        precision highp float h = distf(from + increment * t);
        if(h < minDist)
            return 0.0;
...
precision highp float rand(precision highp float x)
{
    return fract(sin(x) * 43758.5453);
}

precision highp float triangle(precision highp float x)
{
	return abs(1.0 - mod(abs(x), 2.0)) * 2.0 - 1.0;
}

precision highp float time;
vec4 getPixel(vec2 p, vec3 from, vec3 increment, vec3 light)
{
	...
	precision highp float diffuse = 1.0 + min(0.0, dot(normalDir, -light));
	precision highp float inshadow = 0.0;//(1.0 - shadow(hitPos, -light)) * SHADOW_DARKNESS;
	...
	precision highp float xs = (rand(time * 6.6) * 0.1 + 0.9);
	precision highp float ys = (rand(time * 6.6) * 0.1 + 0.9);
	precision highp float hatching = max((clamp((sin(p.x * xs * (170.0 + rand(time) * 30.0) +
...
{	
	...
	// camera position
	precision highp float dist = 50.0;
	vec3 ta = vec3(0,0,0);
