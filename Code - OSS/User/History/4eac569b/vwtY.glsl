...
highp float pTorus(highp vec3 p, highp vec2t) {highp vec2q = vec2(length(p.xz)-t.x,p.y); return length(q)-t.y;}
highp float pTorus2(highp vec3 p, highp vec2t) {highp vec2q = vec2(length(p.xy)-t.x,p.z); return length(q)-t.y;}
...
highp float time;
highp vec4 getPixel(highp vec2p, highp vec3 from, highp vec3 increment, highp vec3 light)
{
...
in highp vec2fragCoord;
out highp vec4 fragColor;
void main(  )
{	
	time = floor(iTime * 16.0) / 16.0;
	// pixel position
	highp vec2q = fragCoord.xy / iResolution.xy;
	highp vec2p = -1.0+2.0*q;
	...
	// mouse
    highp vec2mo = iMouse.xy/iResolution.xy;
	highp vec2m = iMouse.xy / iResolution.xy;
