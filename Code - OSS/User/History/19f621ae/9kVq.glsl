...
highp float pTorus(highp vec3 p, vec2 t) {vec2 q = vec2(length(p.xz)-t.x,p.y); return length(q)-t.y;}
highp float pTorus2(highp vec3 p, vec2 t) {vec2 q = vec2(length(p.xy)-t.x,p.z); return length(q)-t.y;}
...
highp float distf(highp vec3 p)
{
	...
	fUnion(d, pRoundBox(highp vec3(0,0,10) + p, highp vec3(21,21,1), 1.0));
	fUnion(d, pSphere(highp vec3(10,10,0) + p, 8.0));
	fUnion(d, pSphere(highp vec3(16,0,4) + p, 4.0));
	fUnion(d, pCapsule(p, highp vec3(10,10,12), highp vec3(15,15,-6.5), 1.5));
	fUnion(d, pCapsule(p, highp vec3(10,10,12), highp vec3(5,15,-6.5), 1.5));
	fUnion(d, pCapsule(p, highp vec3(10,10,12), highp vec3(10,5,-6.5), 1.5));
	fUnion(d, pTorus(highp vec3(15,-15,0) + p, vec2(6,2)));
	fUnion(d, pTorus2(highp vec3(10,-15,0) + p, vec2(6,2)));
	fUnion(d, pRoundBox(highp vec3(-10,10,-2) + p, highp vec3(1,1,9), 1.0));
	fUnion(d, pRoundBox(highp vec3(-10,10,-4) + p, highp vec3(0.5,6,0.5), 1.0));
	fUnion(d, pRoundBox(highp vec3(-10,10,2) + p, highp vec3(6,0.5,0.5), 1.0));
...
highp vec3 normal(highp vec3 p)
{
	...
	highp float m;
    highp vec3 n = highp vec3( (distf(highp vec3(p.x-eps,p.y,p.z)) - distf(highp vec3(p.x+eps,p.y,p.z))),
                   (distf(highp vec3(p.x,p.y-eps,p.z)) - distf(highp vec3(p.x,p.y+eps,p.z))),
                   (distf(highp vec3(p.x,p.y,p.z-eps)) - distf(highp vec3(p.x,p.y,p.z+eps)))
				 );
    return normalize(n);
}

vec4 raymarch(highp vec3 from, highp vec3 increment)
{
...
highp float time;
vec4 getPixel(vec2 p, highp vec3 from, highp vec3 increment, highp vec3 light)
{
...
{	
	...
	highp vec3 ta = highp vec3(0,0,0);
	highp vec3 ro = highp vec3(cos(m.x) * cos(m.y) * dist, sin(m.x) * cos(m.y) * dist, sin(m.y) * dist);
	highp vec3 light = highp vec3(cos(m.x - 2.27) * 50.0, sin(m.x - 2.27) * 50.0, -20.0);
	
	// camera direction
	highp vec3 cw = normalize( ta-ro );
	highp vec3 cp = highp vec3( 0.0, 0.0, 1.0 );
