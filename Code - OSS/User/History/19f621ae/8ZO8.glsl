...
void fUnion(highp inout float d1, highp float d2) {d1 = min(d1,d2);}
highp float pSphere(vec3 p, highp float s) {return length(p)-s;}
highp float pRoundBox(vec3 p, vec3 b, highp float r) {return length(max(abs(p)-b,0.0))-r;}
highp float pTorus(vec3 p, vec2 t) {vec2 q = vec2(length(p.xz)-t.x,p.y); return length(q)-t.y;}
highp float pTorus2(vec3 p, vec2 t) {vec2 q = vec2(length(p.xy)-t.x,p.z); return length(q)-t.y;}
highp float pCapsule(vec3 p, vec3 a, vec3 b, highp float r) {vec3 pa = p - a, ba = b - a;
	highp float h = clamp( dot(pa,ba)/dot(ba,ba), 0.0, 1.0 ); return length( pa - ba*h ) - r;}

highp float distf(vec3 p)
{
...
highp float shadow(vec3 from, vec3 increment)
{
